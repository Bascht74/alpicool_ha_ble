...
**Conclusion (inferred):** For older Plug In Festivals boxes there are two independent user reports of the Alpicool protocol. For the IceCubeX there are none. The probe script `tools/maentum_probe.py` settles this on the device in one minute.
...
| [Gruni22/alpicool_ha_ble](https://github.com/Gruni22/alpicool_ha_ble) | no license file | Practice: SET echo and status arrive in one notification, write with response required |

## 4. Existing projects

| Project | Type | Status | License | Assessment |
|---|---|---|---|---|
| [Gruni22/alpicool_ha_ble](https://github.com/Gruni22/alpicool_ha_ble) | HACS integration | active, last commit 2026-09-25, 52 commits | **none** | most complete HA solution, see [review](REVIEW.md) |
...
1. **Protocol of the IceCubeX** (Sebastian's model): **resolved** on 2026-09-25 with nRF Connect. Response to the query (three notifications, 20 + 20 + 8 bytes):
   ```
