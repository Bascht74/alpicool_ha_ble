...
# The codec lives in the integration. It has no Home Assistant imports, so it is
# loaded straight from its file instead of importing the integration package.
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "custom_components" / "maentum_ble"))
    ...
    async def send(self, packet: bytes) -> None:
        ...
        for start in range(0, len(packet), self._chunk):
            ...
            await self._client.write_gatt_char(
                WRITE_UUID, packet[start : start + self._chunk], response=self._with_response
            )
...
async def _scan(timeout: float, show_all: bool) -> int:
    ...
    for device, adv in found.values():
        ...
        if hint or show_all:
            rows.append(
                (not hint, device.address, adv.local_name or device.name or "", adv.rssi, hint)
            )
...
async def _query(address: str, bind: bool, loop: float | None, verbose: bool) -> int:
    async with Probe(address, verbose) as probe:
        if bind:
            print("Bind sent. Press the button on the box when it shows 'APP'.", file=sys.stderr)
...
def main(argv: list[str] | None = None) -> int:
    """Entry point."""
    parser = argparse.ArgumentParser(prog="maentum_probe.py", description=__doc__.split("\n")[0])
    ...
    try:
        ...
        return asyncio.run(_set_target(args.address, args.zone, args.temp, args.verbose))
