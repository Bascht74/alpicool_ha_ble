...
This module has no Home Assistant or Bluetooth imports so that it can be unit
tested on its own.
        ...
        while True:
            ...
            if start < 0:
                ...
                if len(self._buffer) > keep:
                    _LOGGER.debug("Dropping bytes without header: %s", self._buffer.hex())
                del self._buffer[: len(self._buffer) - keep]
                break
            if start:
                _LOGGER.debug("Dropping bytes before header: %s", self._buffer[:start].hex())
            ...
            if state is ChecksumState.MISMATCH:
                _LOGGER.debug("Checksum mismatch, accepting frame anyway: %s", raw.hex())
            frames.append(Frame(command=raw[3], payload=raw[4:-2], checksum=state, raw=raw))
