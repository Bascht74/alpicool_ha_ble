...
from custom_components.maentum_ble.protocol import (
    ChecksumState,
    Command,
    FrameReader,
    build_bind,
...
BM_STATUS_1 = H("fe fe 15 01 00 01 00 00 f1 14 ec 02 00 00 00 00 00 00 f3 64 0c 03 05 6c")
# BM: status answer before the Set example (battery unknown, 0x7f)
BM_STATUS_2 = H("fe fe 15 01 00 01 00 00 ec 14 ec 02 00 00 00 00 00 00 f7 7f 0b 01 05 83")
...
BM_SET_ANSWER = H("fe fe 15 02 00 01 00 02 ec 14 ec 02 00 00 00 00 00 00 f7 7f 0b 01 05 86")


def _dual_payload() -> bytes:
    """SYN: 28 byte status of a dual-zone fridge."""
    single = bytearray(parse_status(BM_STATUS_1[4:-2]).raw)
    single += bytes([0x04, 0, 0, 0x01, 0, 0, 0, 0, 0x06, 0x01])  # zone 2: target 4, current 6
    return bytes(single)
...
@pytest.mark.parametrize(("byte", "value"), [(0x7F, 127), (0x80, -128), (0xFF, -1), (0x00, 0)])
